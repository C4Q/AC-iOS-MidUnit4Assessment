...
class CardDataStore {
	...
	func addToFavorites(card: Card, andImage image: UIImage) -> Bool  {
		// checking for uniqueness
		let indexExist = favorites.index{ $0.imdbId == card.imdbId }
		...
		let newFavorite = Favorite.init(title: card.title, year: card.year, imdbId: card.imdbId, type: card.type, poster: card.poster, plot: card.plot)
	...
	func storeImageToDisk(image: UIImage, andCard card: Card) -> Bool {
		...
		// 1) save image from favorite photo
		let imageURL = CardDataStore.manager.dataFilePath(withPathName: card.imdbId)
		do {
	...
	func removeFavorite(fromIndex index: Int, andCardImage favorite: Favorite) -> Bool {
